...
try {
    const htmlList = fs.readdirSync('./htmls/').filter(file => path.extname(file).toLowerCase() === '.html')
    htmlList.forEach(htmlFile => {
        try {
            const htmldoc = fs.readFileSync('./htmls/' + htmlFile, 'utf-8')
            const document = new JSDOM(htmldoc).window.document
            document.querySelectorAll('.text').forEach(questionblock => {
    ...
    questionList.forEach(question => {
        fs.appendFileSync('./nqa.csv', question.toFullQuestion())
    })
} catch (error) {
